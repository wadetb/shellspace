...
void ovr_OnLoad( JavaVM * JavaVm_ )
{
	...
	ClassNativeMethod gMethods[] = {
		...
		{ PlatformActivityClass,	{"nativeSetAppInterface", "(Lcom/oculusvr/vrlib/VrActivity;)J", (void*)Java_com_oculusvr_vrlib_PlatformActivity_nativeSetAppInterface } },
	};
...
void ovr_WarpSwap( ovrMobile * ovr, const TimeWarpParms * parms )
{
...
#if 0	// This check was only necessary when we had to support two different
		// lenses for the S5 headsets.
		// NOTE: If executed in a retail path, the shutting down and restarting
		// of TimeWarp will cause ordering issues with queue/dequeue operations
		// regarding the front buffer rendering which may manifest as an OOM error.
	...
	{
		LOG( "Detected Device Change: " );
		LOG( "\t\tProductId (%d,%d)", newSensorDesc.ProductId, ovr->SensorDesc.ProductId );
		LOG( "\t\tVendorId (%d, %d)", newSensorDesc.VendorId, ovr->SensorDesc.VendorId );
		LOG( "\t\tVersion (%d,%d)", newSensorDesc.Version, ovr->SensorDesc.Version );
