...
{
	const Matrix4f mv( m0, m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11, m12, m13, m14, m15 );
...
OCULUS_EXPORT void OVR_AbandonAudioFocus()
{
	...
	ovr_AbandonAudioFocus( up.OvrMobile );
}
...
void OVR_TimeWarpEvent( const int viewIndex )
{
	...
	{
		GLStateSave glstate;	// restore state on destruction
		ovr_WarpSwapLoadingIcon( up.OvrMobile );
	}
	else
	{
		GLStateSave glstate;	// restore state on destruction
...
// Default measurements are averaged between the male and female medians
static const float OVR_DEFAULT_EYE_HEIGHT	= 1.675f;
static const float OVR_DEFAULT_IPD          = 0.064f;
...
OCULUS_EXPORT bool OVR_GetPlayerEyeHeight(float &eyeHeight)
{
	...
	eyeHeight = OVR_DEFAULT_EYE_HEIGHT;
	return true;
}
